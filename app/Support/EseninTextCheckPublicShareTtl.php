...
class EseninTextCheckPublicShareTtl
{
    ...
    public static function labelsForUi(): array
    {
        ...
        foreach (self::allowedDays() as $days) {
            if (self::isUnlimited($days)) {
                $labels[$days] = (string) __('Site monitoring share ttl unlimited');
                continue;
            }
            $labels[$days] = (string) __('Site monitoring share ttl days', ['days' => $days]);
        }
