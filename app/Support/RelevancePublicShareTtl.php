...
class RelevancePublicShareTtl
{
    ...
    {
        $options = config('cabinet-relevance.public_share_ttl_days', [30, 90, 180, 365, 0]);
    ...
    public static function labelsForUi(): array
    {
        ...
        foreach (self::allowedDays() as $days) {
            if (self::isUnlimited($days)) {
                $labels[$days] = (string) __('Relevance share ttl unlimited');

                continue;
            }
            $labels[$days] = (string) __('Relevance share ttl days', ['days' => $days]);
        }
