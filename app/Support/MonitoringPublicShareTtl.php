...
class MonitoringPublicShareTtl
{
    ...
    {
        $options = config('cabinet-monitoring.public_share_ttl_days', [30, 90, 180, 365, 0]);
    ...
    public static function labelsForUi(): array
    {
        ...
        foreach (self::allowedDays() as $days) {
            if (self::isUnlimited($days)) {
                $labels[$days] = (string) __('Monitoring share ttl unlimited');
                continue;
            }
            $labels[$days] = (string) __('Monitoring share ttl days', ['days' => $days]);
        }
