...
HTML;
    ...
    private function seedSiteAudit(int $userId): string
    {
        ...
        $existing = SiteAuditProject::query()
        ...
        $project = SiteAuditProject::query()->create([
            ...
            'settings_json' => json_decode($data['project']['settings_json'], true),
        ]);
        ...
        unset($crawlPayload['user_id']);
        $crawl = SiteAuditCrawl::query()->create(array_merge($crawlPayload, [
    ...
    private function purgeSiteAuditDomain(int $userId, string $domain): void
    {
        if (! Schema::hasTable('site_audit_projects')) {
            return;
        }

        $projectIds = SiteAuditProject::query()
            ->where('user_id', $userId)
            ->where('domain', $domain)
            ->pluck('id');
