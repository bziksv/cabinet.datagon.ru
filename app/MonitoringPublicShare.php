...
class MonitoringPublicShare extends Model
{
    protected $table = 'monitoring_public_shares';
    ...
    public static function tableAvailable(): bool
    {
        if (self::$tableAvailable === null) {
            try {
                self::$tableAvailable = Schema::hasTable('monitoring_public_shares');
            } catch (\Throwable $e) {
    ...
    {
        if ($this->isUnlimited()) {
            return (string) __('Monitoring share ttl unlimited');
        }
    ...
    public function ttlDaysFromPayload(): int
    {
        ...
        if (isset($meta['ttl_days'])) {
            return MonitoringPublicShareTtl::normalize($meta['ttl_days']);
        }

        return $this->isUnlimited() ? MonitoringPublicShareTtl::UNLIMITED : 30;
    }

    public function publicUrl(): string
    {
        return url('/public/share/monitoring-v2/' . $this->token);
    }
    ...
    public static function reportHash(array $report): string
    {
        return hash('sha256', json_encode([
            ...
            'snapshot_at' => $report['summary']['snapshot_at'] ?? null,
            'words' => $report['summary']['words'] ?? null,
            'top10' => $report['summary']['top10'] ?? null,
            'engines' => count($report['engines'] ?? []),
        ]));
    }
    ...
    public static function issueForProject(
        ...
        $ttlDays = MonitoringPublicShareTtl::normalize($ttlDays);
        ...
        return static::create([
            ...
            'snapshot_hash' => $hash,
            'expires_at' => MonitoringPublicShareTtl::resolveExpiresAt($ttlDays),
        ]);
    }
